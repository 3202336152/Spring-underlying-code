...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory implements AutowireCapableBeanFactory {
    ...
    private Object initializeBean(String beanName, Object bean, BeanDefinition beanDefinition) {
        // invokeAwareMethods：调用 Aware 接口的方法，注入相关的依赖
        if (bean instanceof Aware) {
            // 如果 Bean 实现了 BeanFactoryAware 接口，将当前 BeanFactory 注入到 Bean 中
            if (bean instanceof BeanFactoryAware) {
                ((BeanFactoryAware) bean).setBeanFactory(this);
            }
            // 如果 Bean 实现了 BeanClassLoaderAware 接口，将当前 BeanClassLoader 注入到 Bean 中
            if (bean instanceof BeanClassLoaderAware){
                ((BeanClassLoaderAware) bean).setBeanClassLoader(getBeanClassLoader());
            }
            // 如果 Bean 实现了 BeanNameAware 接口，将当前 Bean 的名称注入到 Bean 中
